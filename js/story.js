(function(){
  ...
  fetch('/src/story.json',{cache:'no-store'})
    .then(function(r){
    ...
    .then(function(stories){
      ...
      stories.forEach(function(item,index){
        ...
        var storyNumber=stories.length-index;
        ...
        if(item.preview&&item.preview.trim()){
          ...
          img.alt=item.judul&&item.judul.trim()
            ?item.judul
            :'Story '+String(storyNumber).padStart(2,'0');
          img.loading='eager';
        ...
        number.textContent='STORY '+String(storyNumber).padStart(2,'0');
        ...
        idx.textContent='CZ / '+String(storyNumber).padStart(3,'0');
        ...
        if(item.link&&item.link.trim()){
          ...
          a.textContent=(item.namabtn&&item.namabtn.trim()
            ?item.namabtn
            :'Read Story')+' →';
